...
$VARS{RUNNING_MODE} = "Rank possible targets in a given nuclear sequence";
$VARS{RUNNING_PARAMS_TEXT} = ""; #to be continued
...
###### Name of the program.
$VARS{crista_exec_file} = "/bioseq/crista/CRISTA_online/search_in_sequence.py";

###### form variables
$VARS{genomicSeq} = $genomicSeq;
$VARS{genomicSeqFile} = $VARS{WorkingDir}."genomicSeq.txt";

###### here we set the html output file (where links to all files will be)

$VARS{OutHtmlFile} = $VARS{WorkingDir}."output.php";
$VARS{RUNNING_PARAMS_TEXT} = $VARS{RUNNING_PARAMS_TEXT}."Genomic sequence: <a href=\"".$VARS{WWWdir}."genomicSeq.txt\">view attached file</a><br>";
...
sub process_main {
    ...
    ###### write genomicSeq to a file
    open (my $FILE, ">", $VARS{genomicSeqFile}) or die "Can't open FILE '$VARS{genomicSeqFile}' $!";
    print $FILE $VARS{genomicSeq};
    close ($FILE);
...
sub run_calc {
    my $crista_comm;
    $crista_comm = "$VARS{crista_exec_file} -f $VARS{genomicSeqFile}";
